...
class TicketController {
    ...
    async index({ request, response }) {
        const tickets = await Ticket.query().with('users').where('status', 'Ativo').fetch();
        response.status(200).json({ tickets: tickets });
    }
    

    async show({ params, request, response }) {
        const ticket = await Ticket.query().with('users').where('id', params.id).fetch();
    ...
    async my_tickets({ params, request, response }) {
        const tickets = await Ticket.query().with('users').where('user_id', params.id).orderBy('tickets.created_at','desc').fetch();
        return response.status(200).json({ tickets: tickets })
    }

    async show_ticket({ params, request, response }) {
        const ticket = await Ticket.query().with('users').where('code', params.code).fetch();
        return response.status(200).json({ ticket: ticket })
    }

    async has_active_ticket({ params, request, response }) {
        console.log(params.id)
        const tickets = await Ticket.query().with('users').where('user_id', params.id).where('status', 'Ativo').first();
        return response.status(200).json({ tickets: tickets })
    }
}
